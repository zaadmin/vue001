...
//每次刚进入网站， 肯定会 调用 main.js ， 在刚调用的时候， 先从本地存储中， 把 购物车的数据读出来，放到store中
 var car = JSON.parse(localStorage.getItem('car') || '[]' );
var store = new Vuex.Store({
    state: { //this.$store.state.***
        car: car//将 购物车中商品的数据，用一个数组存储起来，在car 数组中,存储一些商品的对象，可以暂时将这个商品对象，
                //设计为这样： {id: 商品的id, cont: 要购买的数量， price: 商品的单价， selected: false }
    },
    mutations: { //this.$store.commit('方法名称', '按需传递唯一的参数')
        addToCar( state, goodsinfo) {
            //点击加入购物车，把商品信息 保存到 store 中的car 上
            // 1. 如果购物车中，之前已经有对应的商品了，则只需要更新数量
            ...
            state.car.some(item => {
              ...
              // 如果最终循环完毕，还是 flag=false,则把商品数据直接 push 到 购物车中
            if(!flag){
